...
def detect_gaps(session: Session, today: str) -> dict | None:
    ...
    if max_tx:
        coverage_end = date.fromisoformat(max_tx)
    elif last_append_at:
        coverage_end = date.fromisoformat(last_append_at[:10])
    else:
        return None

    if today_date - coverage_end <= timedelta(days=1):
        return None
    suggested_from = (coverage_end + timedelta(days=1)).isoformat()
    return {
